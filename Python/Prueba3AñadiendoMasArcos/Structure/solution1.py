...
def evaluate(sol):
    ...
    if len(used_arcs) == 0: # si no se usó ningún arco, consideramos fiabilidad perfecta (R=1) para evitar -inf
        ...
        gr = -log_product
    return Q, coste, eq, gr
    
def aggregatedObjective(Q, coste, eq, gr, Q_target, cost_target, pesos, inst):
    ...
    dv4 = max(0.0, gr - inst['t_GR'] ) # fiabilidad es mejor cuanto menor, así que la desviación es gr - meta, ademas esta fijada por 1.2
...
def isFeasible(sol):
    ...
    """
    inst = sol['instance']
    used_supply = {d: 0.0 for d in inst['depositos']}
    received = {j: 0.0 for j in inst['nodes']}
    veh_used = {d: {'small':0, 'medium':0, 'large':0} for d in inst['depositos']}
    total_cost = 0.0
...
def printSolution(sol):
    ...
    # --- Desglose de vehículos y carga por nodo ---
    from collections import defaultdict
    vehiculos_por_nodo = defaultdict(lambda: {'small': 0, 'medium': 0, 'large': 0, 'tons': 0.0})
    
    for env in sol['envios']:
        dep, node, vtype, tons = env
        n_veh = math.ceil(tons / inst['veh_cap'][vtype])
        vehiculos_por_nodo[node]['tons'] += tons
        vehiculos_por_nodo[node][vtype] += n_veh

    print("\nDesglose por nodo de destino:")
    for node in sorted(vehiculos_por_nodo.keys()):
        info = vehiculos_por_nodo[node]
        print(f"  N{node}: recibe {info['tons']:.1f} t con ", end="")
        partes = []
        for vt in ['small', 'medium', 'large']:
            if info[vt] > 0:
                partes.append(f"{info[vt]} {vt}")
        print(", ".join(partes))

    # --- Total global de vehículos ---
    total_veh = {'small': 0, 'medium': 0, 'large': 0}
    for env in sol['envios']:
        dep, node, vtype, tons = env
        n_veh = math.ceil(tons / inst['veh_cap'][vtype])
        total_veh[vtype] += n_veh

    print("\nTotal de vehículos utilizados:")
    for vt in ['small', 'medium', 'large']:
        print(f"  {vt}: {total_veh[vt]}")

    print(f"\nValor objetivo agregado: {sol['of']:.6f}")